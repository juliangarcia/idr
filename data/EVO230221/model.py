...
def two_communities_graph(number_of_agents, initial_number_of_0_tags, density):

    if density == "high":
        rewire_amount = number_of_agents//2
        add_amount = number_of_agents//2
    else:
        rewire_amount = number_of_agents//4
        add_amount = number_of_agents//4
...
def well_mixed_graph(number_of_agents, initial_number_of_0_tags, density):

    if density == "high":
        remove_amount = number_of_agents//4

    else:
        remove_amount = number_of_agents//2
    ...
    F = nx.complete_graph(number_of_agents)
    nx.to_directed(F)

    for _ in range(remove_amount):
        edges_F = [edge for edge in F.edges]
        edge1 = edges_F[np.random.choice(range(len(edges_F)))]
        F.remove_edge(*edge1)
...
graph_function_map = {
    ...
    "two_communities": lambda number_of_agents, number_of_0_tags, density: two_communities_graph(number_of_agents, number_of_0_tags, density),
    "well_mixed": lambda number_of_agents, number_of_0_tags, density: well_mixed_graph(number_of_agents, number_of_0_tags, density),
...
def main(config_file_path):
    ...
    if config["model_type"] == 'evo_network':
        ...
        graph = graph_func(config["number_of_agents"], config["initial_number_of_0_tags"], config["density"])
